...
const cook=(clientId, orders, cookingDone)=>{
    ...
    const iid= setInterval(()=>{
        if(orders.length===0){
            clearInterval(iid);
            return cookingDone(new Error(`Cook Rejects order: No order specified by ${clientId}`)); //return error
        }
...
const waiter=(clientId, orders, serveDone)=>{
        ...
        setTimeout(()=>{
            ...
            if(orders.length===0){
                return serveDone(new Error(`waiter rejects order: No order item specified  ${clientId}`),{clientId});
            }
